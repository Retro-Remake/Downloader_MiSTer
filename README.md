...
Download ConsoleMode.tar.gz and install.sh from the [Release page](https://github.com/Retro-Remake/Downloader_MiSTer/releases/tag/latest), place ConsoleMode.tar.gz in `/media/fat`, install.sh `/media/fat/Scripts`,and run the following commands to install:
...
This tool installs and **updates all the cores** and other extra files for your *MiSTer*. It also updates the menu core, the MiSTer firmware and the Linux system. The source for all downloads is the [MiSTer Distribution](https://github.com/Retro-Remake/Distribution_MiSTer) repository.

### Setup and Usage

This tool is usually pre-installed in your MiSTer. If not, download this [ZIP file](https://github.com/Retro-Remake/Downloader_MiSTer/releases/download/latest/MiSTer_Downloader.zip) and extract `update.sh` to your `/Scripts` folder on your primary SD card (create that folder if it doesn't exist). You only need to perform this operation once, since this tool self-updates itself.
...
NOTE: If you manually add custom databases, you might want to make sure that you also have a [distribution_mister] section. That way you'll still be downloading the content from [MiSTer Distribution](https://github.com/Retro-Remake/Distribution_MiSTer) together with the content from the custom database/s that you introduced.
